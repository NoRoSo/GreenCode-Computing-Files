...
int moveToChessboard(vector<vector<int> > & board){
    int n = board.size();
    ...
    //if it isn't, it means that the board is invalid)
    if(abs(countCol_type1 - countCol_type2) > 1) //if type1 - type2 != -1, 0, 1 then return -1
        return -1;
    if(abs(countRow_type1 - countRow_type2) > 1)
        return -1;
    ...
    int countRow = 0;
    int countCol = 0;
    //count how many moves needed to make the board as a chessboard that starts with 0.
    for(int j = 0; j < n; j++){
        if(board[0][j] != j % 2)
            countCol++;
        if(board[j][0] != j % 2)
    ...
    //count how many moves needed to make the board as a chessboard (horizontally) that start with 1
    ...
    if(countRow2 % 2)
        ...
        if(countRow % 2) //if countRow is an odd number return countRow2 / 2
            res += countRow2 / 2;
        else
            res += min(countRow, countRow2) / 2;

    //if countCol2 is an odd number return countCol/2
    if(countCol2 % 2)
        res += countCol / 2;
    else
        if(countCol % 2) //if countCol is an odd number return countCol2 / 2
            res += countCol2 / 2;
        else
            res += min(countCol, countCol2) / 2;
...
int main(){
    ...
    for(int i = 1; i <= T; i++){
        ...
        cout << "CASE #" << i << ": " << moveToChessboard(board) << endl;
    }
}
